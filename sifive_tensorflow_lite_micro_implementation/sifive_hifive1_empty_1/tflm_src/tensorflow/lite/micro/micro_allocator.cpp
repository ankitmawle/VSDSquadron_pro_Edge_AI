...
//	  std::free(ptr); // Use the standard delete operator
  }
  ...
  *result = {};
  ...
  if (src_quantization && src_quantization->scale() &&
    ...
    if (quantization == nullptr) {
      printf("Unable to allocate TfLiteAffineQuantization.\n");
      return kTfLiteError;
    }
    ...
    if (quantization->zero_point == nullptr) {
      printf("Unable to allocate quantization->zero_point.\n");
      return kTfLiteError;
    }
...
SubgraphAllocations* MicroAllocator::StartModelAllocation(const Model* model) {
  ...
  if (output == nullptr) {
    printf("Failed to allocate memory for model metadata.");
    return nullptr;
  }
...
TfLiteStatus MicroAllocator::RequestScratchBufferInArena(size_t bytes,
  ...
  if (current_node_request_count >= kMaxScratchBuffersPerOp) {
    printf("Scratch buffer request exeeds limit per operator (%d)",
...
TfLiteStatus MicroAllocator::AllocateNodeAndRegistrations(
  ...
  for (size_t subgraph_idx = 0; subgraph_idx < model->subgraphs()->size();
    ...
    if (output == nullptr) {
      printf("Failed to allocate memory for node_and_registrations.");
      return kTfLiteError;
    }
...
TfLiteTensor* MicroAllocator::AllocatePersistentTfLiteTensor(
  ...
  if (tensor == nullptr) {
    printf("Failed to allocate memory for persistent TfLiteTensor");
    return nullptr;
  }
...
TfLiteStatus MicroAllocator::AllocateTfLiteEvalTensors(
  ...
  for (size_t subgraph_idx = 0; subgraph_idx < model->subgraphs()->size();
    ...
    for (size_t i = 0; i < alloc_count; ++i) {
      ...
      if (status != kTfLiteOk) {
        printf("Failed to initialize tensor %d", i);
        return kTfLiteError;
      }
    }
    ...
    if (tensor->is_variable()) {
      if (offline_planner_offsets == nullptr ||
        ...
        if (eval_tensors[i].data.data == nullptr) {
          printf("Failed to allocate variable tensor of size %d",
                      buffer_size);
          return kTfLiteError;
        }
      }
