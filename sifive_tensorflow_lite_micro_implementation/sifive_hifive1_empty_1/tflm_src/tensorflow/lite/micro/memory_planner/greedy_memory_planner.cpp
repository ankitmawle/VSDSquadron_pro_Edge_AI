...
TfLiteStatus GreedyMemoryPlanner::AddBuffer(int size, int first_time_used,
  ...
  if (buffer_count_ >= max_buffer_count_) {
    printf("Too many buffers (max is %d)", max_buffer_count_);
    return kTfLiteError;
  }
...
void GreedyMemoryPlanner::PrintMemoryPlan() {
  ...
  for (int i = 0; i < buffer_count_; ++i) {
    printf("%c (id=%d): size=%d, offset=%d, first_used=%d last_used=%d",
  ...
  for (int t = 0; t <= max_time; ++t) {
    ...
    printf("%s%d: %s (%dk)", t < 10 ? " " : "", t, (const char*)line,
...
TfLiteStatus GreedyMemoryPlanner::GetOffsetForBuffer(int buffer_index,
  ...
  if ((buffer_index < 0) || (buffer_index >= buffer_count_)) {
    printf("buffer index %d is outside range 0 to %d", buffer_index,
                buffer_count_);
    return kTfLiteError;
  }
...
bool GreedyMemoryPlanner::DoAnyBuffersOverlap() {
  ...
  for (int i = 0; i < buffer_count_; ++i) {
    ...
    for (int j = 0; j < buffer_count_; ++j) {
      ...
      printf("Overlap: %d (%d=>%d, %d->%d) vs %d (%d=>%d, %d->%d)", i,
