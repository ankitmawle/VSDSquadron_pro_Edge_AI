    ...
    for (current_operator_index_ = 0; current_operator_index_ < operators_size;
      ...
      if (registration->prepare != nullptr) {
        ...
        if (prepare_status != kTfLiteOk) {
          printf("Node %s (number %uf) failed to prepare with status %d",
                      OpNameFromRegistration(registration),
                      (unsigned)current_operator_index_, prepare_status);
          return kTfLiteError;
        }
      }
...
TfLiteStatus MicroInterpreterGraph::InvokeSubgraph(int subgraph_idx) {
  ...
  if (static_cast<size_t>(subgraph_idx) >= subgraphs_->size()) {
    printf("Accessing subgraph %d but only %lu subgraphs found",
...
// only defined for builds with the error strings.
    ...
    if (invoke_status == kTfLiteError) {
      printf("Node %s (number %u) failed to invoke with status %d",
                  OpNameFromRegistration(registration), (unsigned)current_operator_index_,
                  invoke_status);
      return kTfLiteError;
