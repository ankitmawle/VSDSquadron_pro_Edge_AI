...
RecordedAllocation RecordingMicroAllocator::GetRecordedAllocation(
  ...
  switch (allocation_type) {
    ...
    // the function printf was never reached outside the switch, because
    // each case has a return. As the intention of the printf is to be
    // called when no matching case is found, a default case was added to
    // contemplate an invalid allocation type
    default:
      // printf("Invalid allocation type supplied: %d", allocation_type);
...
void RecordingMicroAllocator::PrintAllocations() const {
  printf("[RecordingMicroAllocator] Arena allocation total %d bytes",
              recording_memory_allocator_->GetUsedBytes());
  printf("[RecordingMicroAllocator] Arena allocation head %d bytes",
              recording_memory_allocator_->GetNonPersistentUsedBytes());
  printf("[RecordingMicroAllocator] Arena allocation tail %d bytes",
