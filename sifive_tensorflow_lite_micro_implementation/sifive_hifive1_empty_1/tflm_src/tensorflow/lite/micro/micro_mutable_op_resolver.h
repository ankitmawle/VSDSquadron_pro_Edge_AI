...
//	  std::free(ptr); // Use the standard delete operator
  ...
  TfLiteStatus AddCustom(const char* name,
    ...
    if (FindOp(name) != nullptr) {
      printf("Calling AddCustom for the same op more than once ");
      printf("is not supported (Op: %s).", name);
      return kTfLiteError;
    }
 ...
 private:
  TfLiteStatus AddBuiltin(tflite::BuiltinOperator op,
    ...
    if (op == BuiltinOperator_CUSTOM) {
      printf("Invalid parameter BuiltinOperator_CUSTOM to the ");
      printf("AddBuiltin function.");
      return kTfLiteError;
    }

    if (FindOp(op) != nullptr) {
      printf("Calling AddBuiltin with the same op more than ");
      printf("once is not supported (Op: #%ld).", op);
      return kTfLiteError;
    }

    if (registrations_len_ >= tOpCount) {
      printf("Couldn't register builtin op #%ld, resolver size ", op);
      printf("is too small (%d).", tOpCount);
      return kTfLiteError;
    }
