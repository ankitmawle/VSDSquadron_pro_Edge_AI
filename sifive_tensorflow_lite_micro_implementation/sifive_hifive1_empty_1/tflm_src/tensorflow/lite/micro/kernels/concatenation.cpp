...
TfLiteStatus ConcatenationPrepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  for (int i = 0; i < num_inputs; ++i) {
    ...
    if (num_dimensions > RuntimeShape::kMaxSmallSize) {
      printf("Op Concatenation does not currently support num dimensions > %d Tensor has %d dimensions.",  RuntimeShape::kMaxSmallSize, num_dimensions);
      return kTfLiteError;
    }
  ...
  switch (output_type) {  // Already know in/outtypes are same.
    ...
    default:
      printf("Concatenation does not currently support Type '%s'.",
