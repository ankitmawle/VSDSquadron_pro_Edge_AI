...
TfLiteStatus GetAxisValueFromTensor(TfLiteContext* context,
  ...
  if (axis_dims > 1) {
    printf("%d Axis dims has only one element for Expand_Dims.", axis_dims);
    return kTfLiteError;
  }
  ...
  if (kTfLiteInt32 == (axis->type)) {
    ...
    *axis_value = axis_ptr[0];
    ...
    printf("Axis type %s (%d) not supported by Expand_Dims.",
...
TfLiteStatus ExpandDimsPrepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (IsDynamicTensor(axis)) {
    printf("DynamicTensor is not yet supported by Expand_Dims.");
    return kTfLiteError;
  }
