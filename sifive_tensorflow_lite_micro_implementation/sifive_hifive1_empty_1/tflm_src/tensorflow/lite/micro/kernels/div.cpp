...
#undef TF_LITE_DIV
    ...
    printf("Unsupported combination of input and output types in DIV.");
    return kTfLiteError;
  }
...
TfLiteStatus DivEval(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (output->type == kTfLiteFloat32) {
    ...
    printf("DIV only supports FLOAT32, INT32, quantized INT8 now, got type %s (%d).", TfLiteTypeGetName(output->type), output->type);
    return kTfLiteError;
  }
