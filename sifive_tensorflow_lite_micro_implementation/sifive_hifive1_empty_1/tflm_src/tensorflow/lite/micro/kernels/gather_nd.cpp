...
TfLiteStatus GatherNdPrepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (params->type) {
    ...
    case kTfLiteInt8:
      break;
    default:
      printf("Params of type '%s' are not supported by gather_nd.",
  ...
  switch (indices->type) {
    case kTfLiteInt32:
      break;
    default:
      printf("Indices of type '%s' are not supported by gather_nd.",
  ...
  if (params_rank < 1) {
    printf("Params must be at least a vector.");
    return kTfLiteError;
  }
  if (indices_rank < 1) {
    printf("Indices must be at least a vector.");
    return kTfLiteError;
  }
  if (indices_nd > params_rank) {
    printf("Index innermost dimension length must be <= params rank.");
    return kTfLiteError;
  }
  if (indices_nd > MAX_INDICES_ND) {
    printf("Index innermost dimension length must not exceed %d.",
                MAX_INDICES_ND);
    return kTfLiteError;
  }
...
TfLiteStatus EvalGatherNd(TfLiteContext* context,
  ...
  switch (params->type) {
    ...
    case kTfLiteInt8:
      ...
      break;
    default:
      printf("Params type '%s' are not supported by gather_nd.",
                  TfLiteTypeGetName(params->type));
      return kTfLiteError;
  }
  if (status != kTfLiteOk) {
    printf("gather_nd index out of bounds");
  }
  return status;
}
