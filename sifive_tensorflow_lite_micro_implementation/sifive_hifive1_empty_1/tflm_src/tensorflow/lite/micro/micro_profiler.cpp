...
void MicroProfiler::Log() const {
  ...
  for (int i = 0; i < num_events_; ++i) {
    ...
    printf("%s took %u ticks (%u ms).", tags_[i], (unsigned)ticks, (unsigned)(TicksToMs(ticks)));
  }
#endif
}
...
#if !defined(TF_LITE_STRIP_ERROR_STRINGS)
  printf("\"Event\",\"Tag\",\"Ticks\"");
  for (int i = 0; i < num_events_; ++i) {
#if defined(HEXAGON) || defined(CMSIS_NN)
    int ticks = end_ticks_[i] - start_ticks_[i];
    printf("%d,%s,%d", i, tags_[i], ticks);
#else
    uint32_t ticks = end_ticks_[i] - start_ticks_[i];
    printf("%d,%s,%" PRIu32, i, tags_[i], ticks);
#endif
  }
#endif
}
...
void MicroProfiler::LogTicksPerTagCsv() {
  ...
  for (int i = 0; i < num_events_; ++i) {
    ...
    printf("%s, %u", each_tag_entry.tag, (unsigned)each_tag_entry.ticks);
  }
  printf("\"total number of ticks\", %d", total_ticks);
#endif
}
